...
function validateTranslationMap(translations,{required=supportedLanguages}={}){
  ...
  return required.filter(language=>{const value=translations[language];return typeof value==='string'?!value.trim():!isPlainObject(value)||typeof value.text!=='string'||!value.text.trim()}).map(language=>'missing translation: '+language);
}
